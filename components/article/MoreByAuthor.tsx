...
const MoreByAuthor = async ({
  ...
  const postsByThisAuthor = await getArticlesByAuthorIdExceptCurrent(authorId, currentPostId);
    ...
    <div className="w-full space-y-6">
      <div className="py-6">
        <div className="flex items-center mb-6 gap-x-5">
          <Ping />
          <h3 className=" text-sm font-medium">Other Articles</h3>
        </div>
        ...
        <div className="grid grid-cols-1 md::grid-cols-2 gap-2">
          {postsByThisAuthor.length == 0 ? (
            ...
            postsByThisAuthor.map((post: CustomPost, i: number) => (
              <div
                key={i}
                className="flex items-center rounded-lg p-3 hover:bg-opacity-70 bg-card transition-colors rounded-lg border  cursor-pointer gap-3"
              >
                ...
                <div className="mr-4 space-y-2">
                  ...
                  >
                    <h4 className="dark:text-white hover:underline line-clamp-1 text-sm font-medium">
                      {post?.title}
                    </h4>
                  </Link>
                  <h4 className="dark:text-gray-50 line-clamp-1 text-xs font-medium">
                    {post.excerpt}
                  </h4>
                </div>
              </div>
